...
import { AddPartToJob } from "@/components/AddPartToJob";
...
export function JobsBoard() {
  const { jobs, jobParts, parts } = useInventory();

  return (
    <main className="space-y-6">
      <AppHeader
        title="Service jobs"
        subtitle="Attach parts by number. Linking a part does not pull it off the shelf."
      />
      <LinkBanner />
      ...
      {jobs.length === 0 ? (
        ...
        <ul className="space-y-4">
          {jobs.map((job) => {
            const required = getJobPartsForJob(job.id, jobParts);
            const missing = required.some((item) => {
              const part = getPartById(item.part_id, parts);
